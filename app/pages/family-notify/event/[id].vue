...
/**
 * /family-notify/event/[id] — picker for a custom event.
 *
 * All photos tagged for this event (by the tablet at upload time) show in
 * one grouped grid. HQ picks any number → '이벤트 발송' fires them all and
...
interface FamilyEvent {
  id: string; name: string; scheduled_date: string;
...
// Reactive picked Set — same pattern as the monthly detail page.
...
// Group photos by resident for clean display.
...
async function togglePick(photo: PhotoCandidate) {
  ...
  // For event photos: "picked" = picked_for_month is non-null. We use a
  // sentinel "EVENT" tag so the cron can ignore it (only month-string-form
  // values trigger month send).
  const sentinel = `event-${eventId.slice(0, 8)}`;
...
async function sendEvent() {
  ...
  try {
    ...
    router.push("/family-notify?tab=events");
  } catch (e: any) {
...
<template>
  <div class="px-8 py-6 max-w-7xl mx-auto">
    <NuxtLink to="/family-notify?tab=events" class="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4">
      ...
      <div v-else class="space-y-6">
        <section v-for="g in grouped" :key="g.resident.resident_id">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            <button v-for="p in g.photos" :key="p.id" type="button" class="group relative rounded-lg overflow-hidden border-2 transition-all aspect-[4/3] focus:outline-none focus:ring-4 focus:ring-primary/30" :class="pickedIds.has(p.id) ? 'border-primary shadow-md ring-2 ring-primary/20' : 'border-transparent hover:border-input'" :disabled="togglingId === p.id" @click="togglePick(p)">
              ...
              <Loader2 v-if="togglingId === p.id" class="absolute top-1.5 right-1.5 h-4 w-4 text-primary animate-spin" />
            </button>
          </div>
        </section>
      </div>
